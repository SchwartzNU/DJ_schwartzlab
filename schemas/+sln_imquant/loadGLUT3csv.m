function [] = loadGLUT3csv(fname, drug_cond, in_vivo_flag)
if nargin < 3
    in_vivo_flag = false;
end
if nargin < 2
...
for i=1:L
    ...
    try
        insert(sln_imquant.GLUT3Stack,stack_key);
    catch
    ...
    for c=1:Ncells
        ...
        if ~isnan(Tpart.GluT3_Top_Integral(c))
            key.glut3_top_surf = Tpart.GluT3_Top_Integral(c);
        end
        if ~isnan(Tpart.GluT3_Bottom_Integral(c))
            key.glut3_bot_surf = Tpart.GluT3_Bottom_Integral(c);
        end
        if ~isnan(Tpart.GluT3_Middle_Integral(c))
            key.glut3_mid = Tpart.GluT3_Middle_Integral(c);
        end
        ...
        insert(sln_imquant.GLUT3Cell,key);
    end
