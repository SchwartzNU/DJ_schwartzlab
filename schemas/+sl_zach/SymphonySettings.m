...
%         that record settings
%
classdef SymphonySettings < dj.internal.GeneralRelvar
    ...
    methods
        ...
        function ret = join(self, arg)
            ...
            ret = init(sl_zach.SymphonySettings, 'join', {self arg});
            ret.operator = 'join';
        ...
        function ret = proj(self, varargin)
            ...
            if nargin > 2 && isa(varargin{1}, 'dj.internal.GeneralRelvar')
                ...
                ret = init(sl_zach.SymphonySettings, 'proj', [{self} varargin]);
                ret.operator = 'proj';
        ...
        function ret = aggr(self, other, varargin)
            assert(iscellstr(varargin), 'proj() requires a list of strings as attribute args')
            ret = init(sl_zach.SymphonySettings, 'aggregate', [{self, other} varargin]);
            ret.operator = 'aggregate';
            ret.operands = [{self, other} varargin];
        end

        function ret = or(self, arg)
            assert(isa(arg, 'sl_zach.SymphonySettings'), 'cannot unify inputs. Did you mean to switch the operand order?');
            ...
            %             end
            ret = init(sl_zach.SymphonySettings, 'union', [{self} {arg}]);
            ret.operator = 'union';
            ret.operands = [{self} {arg}];
        end
        ...
        function ret = and(self, arg)
            ...
            if isa(arg, 'dj.internal.GeneralRelvar') &&~isa(arg, 'sl_zach.SymphonySettings')
                %                 th = arg.tableHeader;
                newarg = init(sl_zach.SymphonySettings, 'wrapper', {arg});
        ...
        function disp(self)
            fprintf('displaying via SymphonySettings');
        ...
        function [header, sql] = compile(self, enclose)
            ...
            switch self.operator
                ...
                case 'table'% terminal node
                    ...
                    %relations do not
                    tab = self;
                    header = sl_zach.HeaderSymphonySettings(derive(tab.tableHeader), tab.className);
                    sql = tab.fullTableName;
                case 'wrapper'
                    %                     [header, sql] = compile(self.operands{1},1);
                    header = sl_zach.HeaderSymphonySettings(derive(self.operands{1}.tableHeader));
                ...
                case 'proj'
                    ...
                    if isa(sql, 'cell')
                        [sql, aliasCount] = header.enclose(sql, aliasCount);
                    end
                ...
                case 'aggregate'
                    ...
                    if ~isa(self.operands{1}, 'sl_zach.SymphonySettings')
                        header = sl_zach.HeaderSymphonySettings(self.operands{1}.header);
                        sql = self.operands{1}.sql;
                    else
                        [header, sql] = compile(self.operands{1}, 2);
                    end

                    if ~isa(self.operands{2}, 'sl_zach.SymphonySettings')
                        header2 = sl_zach.HeaderSymphonySettings(self.operands{2}.header);
                ...
                case 'join'
                    ...
                    if ~isa(self.operands{1}, 'sl_zach.SymphonySettings')
                        ...
                        header1 = sl_zach.HeaderSymphonySettings(self.operands{1}.header);
            ...
            if ~isempty(self.restrictions)
                ...
                if any(isPer)
                    [sql,aliasCount] = header.enclose(sql, aliasCount, per);
                    %                     header
