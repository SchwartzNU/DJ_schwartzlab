%
% NOT a DJ table... an override for the DJ header for SymphonySettings
%
classdef HeaderSymphonySettings < handle
    ...
    methods
        function self = HeaderSymphonySettings(header, className)
            if ~nargin
                return
            end
            ...
            if nargin > 1
                self.settingType = erase(className,'sl_zach.SymphonySettings');
            end
...
%             end
        ...
        function sql = sql(self)
            ...
            for i = 1:length(self.attributes)
                for j=1:1+isUnion
                    if strcmp(self.attributes(i).type, 'fake') && ~isempty(self.headers{j}.settingType)
%                         if isempty(self.headers{j}.settingType)
%                             sql{j} = sprintf('%s,`%s`', sql{j}, self.attributes(i).name);
%                         else
                            sql{j} = sprintf('%s,''%s'' as `%s`', sql{j}, self.headers{j}.settingType, self.attributes(i).name);
%                         end
        ...
        function ret = union(self, other)
            ...
            ret = sl_zach.HeaderSymphonySettings(self);
            ...
            %             ret.header = derive(
            if ~ismember('setting_type',{ret.attributes.name})
                f = fieldnames(ret.attributes);
                attr = cell2struct(cell(size(f)), f);
                attr.name = 'setting_type';
                attr.type = 'fake';
                attr.iskey = true;
            ...
            for iAttr=1:length(params)
                if strcmp('*',params{iAttr})
                    ...
                    if ~isempty(toks)
                        ...
                        if ~isempty(toks)                            
                            if contains(toks{1}{1},'=BLOB=') %Greg 1/24/21 - fix for how DJ 3.4.1 deals with BLOBs
                                %do nothing?
                                include(ix)=true;
                            else % process a computed attribute
                                ix = self.count + 1;
                                self.attributes(ix) = struct(... %TODO: only verified to work on aggr(count
                                    'name', toks{1}{2}, ...
        ...
        function ret = join(hdr1, hdr2)
            ...
            % merge primary keys
            ret = sl_zach.HeaderSymphonySettings();
        ...
        function clause = makeWhereClause(header, restrictions)
            ...
            for arg = restrictions
                cond = arg{1};
                switch true
                    case isa(cond, 'sl_zach.SymphonySettings') && strcmp(cond.operator, 'union')
                        % union
                    ...
                    case isa(cond, 'sl_zach.SymphonySettings') && strcmp(cond.operator, 'not')
