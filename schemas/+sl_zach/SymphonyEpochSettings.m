...
properties (Constant, Access=private)
  ...
  blobs = sl_zach.SymphonyEpochParameterBlob().fetch('parameter_name');
...
%       hashes = do_hash(values(blob_i,u(N)), names(blob_i));
      ...
      if ~isempty(c{1})
        if isnan(q)
          q = sl_zach.SymphonyEpochSettingsBlob & c; 
        else
          q = q | (sl_zach.SymphonyEpochSettingsBlob & c); 
        end
      end
...
%     keys_STRING(dups,:) = [];
    ...
    sl_zach.SymphonyEpochSettingsBlob().insert(keys_BLOB);
