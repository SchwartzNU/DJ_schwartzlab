...
%NOTE THIS FUNCTION AND TBALE ARE INACTIVE, DO NOT USE
    ...
    %                 len_each(i) = len_each(i) + sln_image.AxonImageMorphV2.eudistance(p1, p2, ...
    %                     scales.x_scale, scales.y_scale, scales.z_scale);
    % 
    %                 %density vs depth
    %                 p1_2d = [bundle.x(j) bundle.y(j)];
    %                 p2_2d = [bundle.x(paidx) bundle.y(paidx)];
    %                 [pjp1x, pjp1y] = sln_image.AxonImageMorphV2.project_to_ax(p1_2d(1), p1_2d(2), ...
    ...
    % 
    %                 [pjp2x, pjp2y] = sln_image.AxonImageMorphV2.project_to_ax(p2_2d(1), p2_2d(2),...
    %                     trace.axon_axis.slope, trace.axon_axis.intercept);
    ...
    %         C = dj.conn;
    %         C.startTransaction;
    %         insert(sln_image.AxonImageMorphV2, key);
    %         C.commitTransaction;
    % 
