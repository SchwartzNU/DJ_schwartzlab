...
%         that record events
%
classdef CellEvent < dj.internal.GeneralRelvar
    ...
    methods (Static)
        ...
        function query = all()
            ...
            for table = tables
                ...
                if any(strcmp({m.SuperclassList(:).Name}, 'sl.CellEvent'))
    ...
    methods
        ...
        function ret = join(self, arg)
            ...
            ret = init(sl.CellEvent, 'join', {self arg});
            ret.operator = 'join';
        ...
        function ret = proj(self, varargin)
            ...
            if nargin > 2 && isa(varargin{1}, 'dj.internal.GeneralRelvar')
                ...
                ret = init(sl.CellEvent, 'proj', [{self} varargin]);
                ret.operator = 'proj';
        ...
        function ret = aggr(self, other, varargin)
            assert(iscellstr(varargin), 'proj() requires a list of strings as attribute args')
            ret = init(sl.CellEvent, 'aggregate', [{self, other} varargin]);
            ret.operator = 'aggregate';
            ret.operands = [{self, other} varargin];
        end

        function ret = or(self, arg)
            assert(isa(arg, 'sl.CellEvent'), 'cannot unify inputs. Did you mean to switch the operand order?');
            ...
            %             end
            ret = init(sl.CellEvent, 'union', [{self} {arg}]);
            ret.operator = 'union';
            ret.operands = [{self} {arg}];
        end
        ...
        function ret = and(self, arg)
            ...
            if isa(arg, 'dj.internal.GeneralRelvar') &&~isa(arg, 'sl.CellEvent')
                %                 th = arg.tableHeader;
                newarg = init(sl.CellEvent, 'wrapper', {arg});
        ...
        function [header, sql] = compile(self, enclose)
            ...
            switch self.operator
                ...
                case 'aggregate'
                    ...
                    if ~isa(self.operands{1}, 'sl.CellEvent')
                    ...
                    if ~isa(self.operands{2}, 'sl.CellEvent')
...
function [limit, per, args, outer] = makeLimitClause(varargin)
    ...
    if ischar(lastArg) && contains(lastArg, 'PER')
        ...
        if isempty(per.orderby) || strcmp(per.orderby, 'DESC')
            per.orderby = '`entry_time` DESC'; %default sorting
            tokens = {'entry_time'};
        elseif strcmp(per.orderby, 'ASC')
            per.orderby = '`entry_time` ASC';
            tokens = {'entry_time'};
        else
