...
%}
classdef FlashedBarCARunner < dj.Computed
    properties
        keySource = sln_symphony.Dataset & ...
            ...
            'protocol_name="flashed_bar"' & ...
            'amp_mode="Cell attached"')
    end

    methods(Access=protected)
        function makeTuples(self, key)
            key.analysis_name = 'FlashedBar_CA';
            q = sln_results.DatasetFlashedBarCA & key & 'LIMIT 1 PER source_id ORDER BY entry_time DESC';
            if q.exists
    ...
    methods 
        function err = errors(self)
            err = self.keySource - sln_results.DatasetFlashedBarCA;
        end
    end
end