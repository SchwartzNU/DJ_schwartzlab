...
git_tag : varchar(128)
%}
classdef DGspeedtuningRunner < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            key.analysis_name = 'DG_speed_tuning_by_cell',
            R = DG_speed_tuning_by_cell(key);
            sln_results.insert(R, 'Dataset','false');
            q = sln_results.DatasetDGspeedtuningbycell & key & 'LIMIT 1 PER source_id ORDER BY entry_time DESC';
            if q.exists
    ...
    methods 
        function err = errors(self)
            err = self.keySource - sln_results.DG_speed_tuning_by_cell;
        end
    end
end