function S = changeCase(S, caseType)
  % changeCase Case converter.
  %
  %  changeCase(S, caseType) converts the case of string S to caseType.
  %   can be a single string, or a cell/char array of strings. New lines or
  %   carriage returns in the string will be converted to white space.
  %
  %    |-------------|------------------|------------------|
  ...
  if iscell(S)
    S = cellfun(@(x) changeCase(S, caseType),...
