...
if recompute
    ...
    load(['result_tables' filesep 'NF1_SMS_results'],'T');
end
%% separate sub-tables
...
%% Load the light step flashes data and count the cells
load DataGroups/LS_flashes_NF1.mat;

q = sln_results.DatasetLSflashesCA & proj(data_group);
R = sln_results.toMatlabTable(q);
N = height(R);

fprintf("-----------\n");
fprintf("%d total cells with LightStep flashes data\n", N);

%% set up table to hold results and count spikes and get peak FRs
if recompute
    ...
    for i=1:N
        ...
        T.peak_FR(i) = max(R.psth_y(i,:));
        T.spike_count(i) = R.spikes_mean(i);
    end
    save(['result_tables' filesep 'NF1_LS_flash_results'],'T');
else
    load(['result_tables' filesep 'NF1_LS_flash_results'],'T');
end

%% separate by genotype and get some numbers
ind_het = strcmp(T.genotype, 'NF1(NF1-KO/WT)');
T_het = T(ind_het,:);
T_wt = T(~ind_het,:);

fprintf("-----------\n");
fprintf("Peak firing rate het = %f +/- %f Hz (1 s.d.), N = %d\n", ...
    mean(T_het.peak_FR), std(T_het.peak_FR), length(T_het.peak_FR));
fprintf("Peak firing rate WT = %f +/- %f Hz (1 s.d.), N = %d\n", ...
    mean(T_wt.peak_FR), std(T_wt.peak_FR), length(T_wt.peak_FR));

[~,p] = ttest2(T_het.peak_FR, T_wt.peak_FR);
fprintf("Distributions different? p = %f\n", p);

fprintf("-----------\n");
fprintf("Spike count het = %f +/- %f Hz (1 s.d.), N = %d\n", ...
    mean(T_het.spike_count), std(T_het.spike_count), length(T_het.spike_count));
fprintf("Spike count WT = %f +/- %f Hz (1 s.d.), N = %d\n", ...
    mean(T_wt.spike_count), std(T_wt.spike_count), length(T_wt.spike_count));

[~,p] = ttest2(T_het.spike_count, T_wt.spike_count);
fprintf("Distributions different? p = %f\n", p);
