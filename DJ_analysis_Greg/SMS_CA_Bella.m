function R = SMS_CA_Bella(data_group, params)
...
R = sln_results.table_definition_from_template('SMSCABella',N_datasets);
...
for d=1:N_datasets
    ...
    spikes_pre_var = zeros(N_spot_sizes,1);
    spikes_stim_var = zeros(N_spot_sizes,1);
    spikes_tail_var = zeros(N_spot_sizes,1);
    ...
    for s=1:N_spot_sizes
        ...
        spikes_pre_var(s) = var(pre_spikes);
        spikes_stim_var(s) = var(stim_spikes);
        spikes_tail_var(s) = var(tail_spikes);
        spikes_stim_sem(s) = std(stim_spikes)./sqrt(N_epochs_per_size(s)-1);
        spikes_tail_sem(s) = std(tail_spikes)./sqrt(N_epochs_per_size(s)-1);
        baseline_rate_all=[baseline_rate_all; pre_spikes / (pre_stim_tail.pre_time / 1E3)];
    end
    ...
    R.spikes_pre_var{d} = spikes_pre_var;
    R.spikes_stim_var{d} = spikes_stim_var;
