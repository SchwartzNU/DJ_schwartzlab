function required_fields = plot_DriftingGratings_MultiCondition_CC_peak_neg(R,condition_struct,ax)
if nargin < 1
    required_fields = {'cycle_avg_peak_neg', ...
        'speeds', 'directions', 'halfwidths', 'contrasts'};
    return;
end

val_set = R.cycle_avg_peak_neg;
...
if polar
    ...
    polarplot(ax, angles, vals, ...
        ...
        'LineWidth',2);
    legend(ax, {'Cycle avg. positive peak (mV)'},'Location','southwest');
else
    plot(ax, x_vals, values,'Color','k','LineStyle','-','Marker','o','MarkerFaceColor','k','LineWidth',2);
    xlabel(ax, x_title);
    ylabel(ax, 'Cycle avg. positive peak (mV)');
end
