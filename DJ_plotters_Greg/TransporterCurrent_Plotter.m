function required_fields = plot_TransporterCurrent(R, ax)
    if nargin < 1
    ...
    [num_rows, num_cols] = size(traces);
    stim_end_idx = R.stim_end_idx;
    ...
    for s = 1:num_rows
        ...
        title(ax, ['Long pulse with AHP area, Current: ' num2str(current, '%.2f pA') ', Spikes: ' num2str(spike_count)]);
        v_line = xline(ax, x(stim_end_idx), 'g-', 'LineWidth', 2);
        h_line = yline(ax, vrest(s), 'r-', 'LineWidth', 2);
        lgd = legend(ax, [h_trace, h_fill, v_line, h_line], {'Trace', ['Shaded AHP Area: ' num2str(ahp_area, '%.2f')], 'Stimulus End', ['Resting Voltage: ' num2str(vrest(s), '%.2f') ' mV']}, 'Location', 'best');
        hold(ax, 'off');

        %% Zoomed-in view
        subplot(ax, 2, 1, 2);  % Subplot for zoomed-in view
        plot(ax, x, trace, 'b');  % Repeat plotting of the trace
        hold(ax, 'on');
        fill(ax, [x_fill, fliplr(x_fill)], [y_fill, fliplr(vrest_line(1:step_size:end))], 'b', 'FaceAlpha', 0.3);  % Repeat shading

        % Set x and y limits to zoom in on the shaded area
        xlim(ax, [min(x_fill), max(x_fill)]);
        ylim(ax, [min(y_fill) - 5, max(y_fill) + 5]);  % Adjust padding as needed

        xlabel(ax, 'Time (s)');
        ylabel(ax, 'Voltage (mV)');
        title(ax, 'Zoomed View of AHP');
        hold(ax, 'off');
    end
end
