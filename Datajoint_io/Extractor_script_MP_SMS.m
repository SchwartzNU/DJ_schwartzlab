...
MP_data_RGCs = ((sln_results.DatasetMultiPulsevaryCurrentFeatureExtract * ...
    proj(sln_animal.Animal,'source_id->animal_source','*') * ...
    sln_animal.GenotypeString * ...
    sln_symphony.UserParamCellCcQuality * ...
    sln_cell.CellName * ...
    sln_cell.Cell * ...
    ...
    & 'NOT animal_id in (3768,3788,3789)') ... %control animals infused radioactive w Shawn
    - proj(sln_symphony.UserParamAnimalFeedingCondition & 'feeding_condition = "HFD" OR feeding_condition = "FR"') & 'cell_unid NOT between 10988 and 10997';
...
%     'dataset_name NOT LIKE "%Wash%"' & ...
%     'dataset_name NOT LIKE "%functionalImaging%"' & ...
%     'dataset_name NOT LIKE "%HighRstar%"' & ...
%     'dataset_name NOT LIKE "%green%"' & ...
%     'dataset_name NOT LIKE "%longPre%"' & ...
%     'dataset_name NOT LIKE "%fromMean%"' & ...
%     'dataset_name NOT LIKE "%darkspot%"' & ...
%     'dataset_name NOT LIKE "%MFA%"' & ...
%     'dataset_name NOT LIKE "%Z1_8Hz%"' & ...
...
SMS_CC_ACs = sln_results.DatasetSMSCC * ...
    proj(sln_animal.Animal,'source_id->animal_source','*') * ...
    sln_animal.GenotypeString * ...
    sln_symphony.UserParamCellDacDataset * ...
    sln_cell.CellName * ...
    sln_cell.Cell * ...
    proj(sln_cell.AssignType.current,'*','entry_time->type_assignment_entry_time','user_name->type_assignment_user_name') & ...
    'cell_class="amacrine"' & ...
...
%% get MP data
% MP_data_ACs = sln_results.DatasetMultiPulsevaryCurrentFeatureExtract * ...
%     aka.Dataset * ...
%     proj(sln_animal.Animal,'source_id->animal_source','*') * ...
%     sln_animal.GenotypeString * sln_cell.Cell * ...
%     sln_symphony.UserParamCellLabeled * ...
...
%% Get matching ones and extract them
ids = unique(fetchn(MP_data_ACs,'cell_unid'));

z=1;
data = struct;
for i=1:length(ids)
    i
    q_MP = MP_data_ACs & sprintf('cell_unid=%d',ids(i));
    if q_MP.count == 1
        q_SMS = SMS_CC_ACs & sprintf('cell_unid=%d',ids(i));
        if q_SMS.count == 1
            data(z).current_injections = fetch(q_MP,'*');
            epochs = sln_symphony.DatasetEpoch * ...
                sln_symphony.ExperimentChannel * ...
                ...
                proj(q_MP);
            data(z).current_injections.epoch_traces = fetch(epochs,'*');
            data(z).light_responses = fetch(q_SMS,'*');
            data_for_cell = data(z);
            save(sprintf('Cell_%d',ids(i)), 'data_for_cell');
            exportStructToHDF5(data_for_cell,sprintf('Cell_%d.h5',ids(i)),'/');
            z=z+1;
